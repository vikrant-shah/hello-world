...
# 1 to 2 digits followed by . or , followed by one or more (+) digits and %
...
payment_df = df['File_PaymentStartDate'].str.extract(r'((\w{1,2})[-](' + monthshort + ')[-](\d{2}))|((' + monthlong +')[^0-9]?\s*(\w{1,2})[.,]\s*(20\d{2}))', expand = False)
...
payment_df['PaymentStartDate1'] = payment_df['Pat1yyyy'].astype(str) + "-" + payment_df['Pat1Month'].astype(str) + "-1"  #+ payment_df['Pat1dd'].astype(str)
#hardcoded 1 as dd
...
maturity_df = df['File_MaturityDate'].str.extract(r'((\w{1,2})[-](' + monthshort + ')[-]([2-9][2-9]))|((' + monthlong +')[^0-9]\s*(\w{1,2})[.,]\s*(20\d{2}))', expand = False)
...
maturity_df['MaturityDate1'] = maturity_df['Pat1yyyy'].astype(str) + "-" + maturity_df['Pat1Month'].astype(str) + "-1" # + maturity_df['Pat1dd'].astype(str)
#hardcoded 1 as dd
...
df['RPA_PaymentAmount'] = df['RPA_PaymentAmount'].str.extract(r'([$s5]*\d+\.\d{2})',expand = False)
